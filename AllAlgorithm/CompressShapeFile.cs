...
namespace gis_no6_compress
{
    class CompressShapeFile //shapefile的压缩
    {
        ...
        {
            //AllData=(ArrayList)YourData.Clone();
            //AllData.Clear();
            //AllData.AddRange(YourData);
        ...
        /*
        public void DaoGeLaSi(ArrayList pts,int StartIndex,int EndIndex,float maxdis) //pts为坐标对，startindex为起始点索引，endindex为最后点索引，maxdis为阈值
        {
            if (StartIndex +1>= EndIndex) return;
            int MidIndex = 0;
            float dis = float.MinValue;
            ArrayList disall = new ArrayList();
            for (int i = StartIndex + 1; i < EndIndex;i++)//求最大距离
            {
                PointF tmp = (PointF)pts[i];
                float distmp = Dist(tmp, (PointF)pts[StartIndex], (PointF)pts[EndIndex]);
            ...
            //计算阈值
            //disall.Sort();
            //int num = (int)(disall.Count / (100.0 / compress) + 0.5);
            //if (num == 0) return;
            //float limit = (float)disall[num - 1];//确定阈值

            //删除小于阈值的点，令这些点的坐标为0,0
            if(dis<maxdis)
            {
                for(int i=StartIndex + 1; i < EndIndex;i++)
                {
                    PointF tmp = new PointF(0f,0f);
                    pts[i]=tmp;
                }
                return;
            }
            ...
            //递归
            DaoGeLaSi(pts, StartIndex, MidIndex, maxdis);
            DaoGeLaSi(pts, MidIndex, EndIndex, maxdis);
        }
        */
    }
}
